...
// Usa Brevo (Sendinblue) via API HTTP — funciona no Render gratuito,
// permite enviar para qualquer e-mail sem domínio próprio (300/dia grátis)
...
const sendPasswordResetEmail = async ({ to, name, resetUrl }) => {
          ...
          <table width="100%" style="max-width:520px;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
            ...
            <tr>
              <td style="padding:36px 32px;">
                ...
                <p style="margin:0 0 8px;font-size:13px;color:#94a3b8;text-align:center;">Este link expira em <strong>15 minutos</strong>.</p>
                <p style="margin:0;font-size:13px;color:#94a3b8;text-align:center;">Se você não solicitou a redefinição, ignore este e-mail.</p>
              </td>
            </tr>
            <tr>
  ...
  const response = await fetch('https://api.brevo.com/v3/smtp/email', {
    ...
    body: JSON.stringify({
      sender: {
        name: 'User System',
        email: process.env.MAIL_USER, // seu e-mail cadastrado no Brevo
      },
      to: [{ email: to, name }],
...
const verifyMailConnection = async () => {
  ...
  console.log('✅ Brevo (e-mail) configurado.');
};
